...
export class Sprite extends AssetBase implements GetImage {
  ...
  /**
   * Serialize this Sprite back to binary form.
   * Uses a "mirror constructor" strategy: reads from original raw bytes
   * (via getRaw) using a temporary reader, and writes to the output writer
   * in the exact same order. Only pixelsToUnits can be modified.
   */
  serialize(writer: ArrayBufferWriter): void {
    ...
    // rect (4 × float32 = 16 bytes)
    this.copyMove(r, writer, 16);

    // offset (2 × float32 = 8 bytes)
    this.copyMove(r, writer, 8);

    // border (4 × float32 = 16 bytes, version >= 4.5)
    if (version[0] > 4 || (version[0] === 4 && version[1] >= 5)) {
      this.copyMove(r, writer, 16);
    }

    // pixelsToUnits (float32, 4 bytes) — modified
    r.readFloat32(); // skip original
    writer.writeFloat32(this._modifiedPixelsToUnits ?? this.pixelsToUnits);

    // Copy the rest (pivot, extrude, isPolygon, renderDataKey, atlasTags,
    // spriteAtlas, spriteRenderData) verbatim from the original raw bytes.
    const remaining = rawBytes.byteLength - r.position;
    if (remaining > 0) {
      writer.writeBuffer(r.readUInt8Slice(remaining));
    }
  }
